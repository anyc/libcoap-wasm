...
EMCC_FLAGS+=$(INCLUDES) --bind -s 'EXPORTED_RUNTIME_METHODS=["cwrap"]' \
	...
	-s EXPORT_ALL=1 -s ERROR_ON_UNDEFINED_SYMBOLS=0 -s LEGACY_RUNTIME=1 \
	-sASYNCIFY=1 -Oz -s ALLOW_TABLE_GROWTH \
	--js-library src/libcoap-wasm-lib.js \
	-sMODULARIZE -s 'EXPORT_NAME="libcoapWasmFactory"'

LIBCOAP_MAX_LOGGING_LEVEL=5
ifneq ($(DEBUG),)
LIBCOAP_MAX_LOGGING_LEVEL=8
EMCC_FLAGS+=-s SOCKET_DEBUG=1 -s WEBSOCKET_DEBUG=1
endif

$(LIBCOAP_BUILD_DIR)/libcoap-3.a:
	# This is a single-threaded client; server async and observe persistence are unused.
	emcmake cmake -B $(LIBCOAP_BUILD_DIR) -S $(LIBCOAP_SRC_DIR) \
		...
		-DENABLE_SERVER_MODE=OFF -DENABLE_PROXY_CODE=OFF -DENABLE_ASYNC=OFF \
		-DWITH_OBSERVE_PERSIST=OFF -DENABLE_THREAD_SAFE=OFF \
		-DMAX_LOGGING_LEVEL=$(LIBCOAP_MAX_LOGGING_LEVEL) \
		-DCMAKE_INSTALL_PREFIX=/usr/local \
		-DCMAKE_BUILD_TYPE=release -DCMAKE_C_FLAGS_RELEASE='-Oz -DNDEBUG'
