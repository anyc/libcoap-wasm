...
test('Explorer discovers resources and exchanges GET, PUT, and file upload requests',
  {timeout: 60000}, async () => {
    ...
    try {
      ...
      const input = resource.locator('input[type="text"]');
      ...
      const fileText = `file-upload-${Date.now()}-` + 'x'.repeat(16384);
      await resource.locator('input[type="file"]').setInputFiles({
        name: 'upload.txt', mimeType: 'text/plain', buffer: Buffer.from(fileText),
      });
      await resource.getByRole('button', {name: 'Upload file'}).click();
      await page.waitForFunction(() => {
        const row = [...document.querySelectorAll('.resource')].find(element =>
          [...element.querySelectorAll('.attribute-value')].some(attribute =>
            attribute.textContent === '/example_data'));
        return row?.querySelector('.resource-status')?.textContent?.startsWith('Upload 2.');
      }, null, {timeout: 20000});
      await resource.getByRole('button', {name: 'Get', exact: true}).click();
      await page.waitForFunction(expected => {
        const row = [...document.querySelectorAll('.resource')].find(element =>
          [...element.querySelectorAll('.attribute-value')].some(attribute =>
            attribute.textContent === '/example_data'));
        return row?.querySelector('input[type="text"]')?.value === expected;
      }, fileText, {timeout: 20000});
